...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('_astro/Layout.0NeSWDeX.css') }}">
    ...
    <div class="footer_body">
        <div class="footer_center">
            <div class="footer_center-grid">
                <div class="footer_center-item">
                    <div class="footer_center-title _24"><span>{{ $main->drop_me_heading1 }}</span> <span>{{ $main->drop_me_heading2 }}</span></div>
                    <div class="footer_center-list">
                        @foreach($main->drop_me_links as $link)
                            <a href="{{ $link['url'] }}" target="_blank" class="footer_link">
                                <div class="footer_arrow">
                                    <svg width="100%" height="100%" viewbox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M1.08023 1.98054L1.18205 0.521072L10.2783 0.487132L10.2613 9.60032L8.80184 9.70215L8.81881 2.96483L1.48752 10.2961L0.452319 9.26091L7.76663 1.9466L1.08023 1.98054Z" fill="currentColor"></path>
                                    </svg>
                                </div>
                                <div class="footer_text _24" link-text-parent="">
                                    <span link-text-target="">{{ $link['name'] }}</span>
                                </div>
                            </a>
                        @endforeach
                    </div>
                ...
                <div class="footer_center-item" copy-mail="">
                    <div class="footer_center-title _24"><span>{{ $main->copy_heading }}</span></div>
                    <div class="footer_center-list">
                        @foreach($main->copy_links as $link)
                            <div class="footer_copy _24" link-text-parent="">
                                <span link-text-target="">{{ $link['name'] }}</span>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="footer_center-item">
                    <div class="footer_center-title _24"><span>{{ $main->social_media_heading1 }}</span> <span>{{ $main->social_media_heading2 }}</span></div>
                    <div class="footer_center-list">
                        @foreach($main->social_media_links as $link)
                            <a href="{{ $link['url'] }}" target="_blank" class="footer_link">
                                <div class="footer_arrow">
                                    <svg width="100%" height="100%" viewbox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M1.08023 1.98054L1.18205 0.521072L10.2783 0.487132L10.2613 9.60032L8.80184 9.70215L8.81881 2.96483L1.48752 10.2961L0.452319 9.26091L7.76663 1.9466L1.08023 1.98054Z" fill="currentColor"></path>
        ...
        <div class="footer_bot">
            <div class="footer_bot-left">
                <div class="_24">{{ $main->stas_bondar }}</div>
                <div class="_24">© {{ $main->year }}</div>
            </div>
            <a href="{{ $main->privacy_policy_url ?? '#' }}" class="footer_link">
                <div class="footer_arrow">
                ...
                <div class="footer_text _24" link-text-parent=""><span link-text-target="">{{ $main->privacy_policy }}</span>
                </div>
            </a></div>
    </div>
</footer>
    ...
    <div class="awards_wrapper">
        <div class="awards_container">
            <div class="awards_list _20">
                <div class="awards_list-title">{{ $main->awward_title }}</div>
                <div class="awards_inner-list">
                    @foreach($main->awward_list as $award)
                        <div class="awards_item _18">
                            <span>{{ $award['line1'] }}</span>
                            <span>{{ $award['line2'] }}</span>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="awards_list _20">
                <div class="awards_list-title">{{ $main->css_design_title }}</div>
                <div class="awards_inner-list">
                    @foreach($main->css_design_list as $item)
                        <div class="awards_item _18">
            ...
            <div class="awards_list _20">
                <div class="awards_list-title">{{ $main->the_fwa_title }}</div>
                <div class="awards_inner-list">
                    @foreach($main->the_fwa_list as $award)
                        <div class="awards_item _18">
                            <span>{{ $award['line1'] }}</span>
                            <span>{{ $award['line2'] }}</span>
                        </div>
...
<vercel-analytics data-props="{}" data-params="{}" data-pathname="/contact"></vercel-analytics>
<script type="module">var f = "@vercel/analytics", l = "1.5.0", w = () => {
        window.va || (window.va = function (...r) {
    ...
    function b(e, r) {
        ...
        try {
            ...
            for (const [a, i] of t) if (!Array.isArray(i)) {
                const o = s(i);
    ...
    function y(e) {
        return e.scriptSrc ? e.scriptSrc : c() ? "https://va.vercel-scripts.com/v1/script.debug.js" : e.basePath ? `${e.basePath}/insights/script.js` : "/_vercel/insights/script.js"
    }

    function g(e = {debug: !0}) {
        var r;
        if (!d()) return;
        v(e.mode), w(), e.beforeSend && ((r = window.va) == null || r.call(window, "beforeSend", e.beforeSend));
        const n = y(e);
        if (document.head.querySelector(`script[src*="${n}"]`)) return;
        const t = document.createElement("script");
        t.src = n, t.defer = !0, t.dataset.sdkn = f + (e.framework ? `/${e.framework}` : ""), t.dataset.sdkv = l, e.disableAutoTrack && (t.dataset.disableAutoTrack = "1"), e.endpoint ? t.dataset.endpoint = e.endpoint : e.basePath && (t.dataset.endpoint = `${e.basePath}/insights`), e.dsn && (t.dataset.dsn = e.dsn), t.onerror = () => {
            const a = c() ? "Please check if any ad blockers are enabled and try again." : "Be sure to enable Web Analytics for your project and deploy again. See https://vercel.com/docs/analytics/quickstart for more information.";
            console.log(`[Vercel Web Analytics] Failed to load script from ${n}. ${a}`)
        }, c() && e.debug === !1 && (t.dataset.debug = "false"), document.head.appendChild(t)
    }

    function p({route: e, path: r}) {
        var n;
        (n = window.va) == null || n.call(window, "pageview", {route: e, path: r})
    }
    ...
    customElements.define("vercel-analytics", class extends HTMLElement {
        constructor() {
            super();
            try {
                const r = JSON.parse(this.dataset.props ?? "{}"), n = JSON.parse(this.dataset.params ?? "{}");
                g({
                    ...r,
                    disableAutoTrack: !0,
                    framework: "astro",
...
<script type="module" src="{{ asset('_astro/Layout.astro_astro_type_script_index_0_lang.D0haerwi.js') }}"></script>
</body>
</html>
